...
1:
Inside rabbitmq host exec enable plugin rabbitmq.
rabbitmq-plugins enable rabbitmq_management
Set env in sh script and run or You can set env var in step 3 for local run.

Get LICENSE_KEY from the url https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher

cat > ./SET_RABBITMQ_VARS.sh <<EOF
#!/bin/bash
set -e
export NR_LICENSE_KEY="eu01x**********\*\*\***********a40c01NRAL"
export RABBITMQ_ENDPOINT=queue.your_env.toys #(IP or FQDN)
export RABBITMQ_PORT=15672 #(for web, usually 15672)
export RABBITMQ_USERNAME="user"
export RABBITMQ_PASSWORD="userpass"
export RABBITMQ_USE_SSL=false #(false | true)
export QUEUES_REGEXES='["*"]' # '["queue[0-9]+","._"]'  
export EXCHANGES_REGEXES='["_"]' # '["amq.*","sync.*"]'  
EOF
sh ./SET_RABBITMQ_VARS.sh

3. Local run or use cloud.

docker run --name nri-rmq -d \
-e HOSTNAME=${RABBITMQ_ENDPOINT} \
-e PORT=${RABBITMQ_PORT} \
-e RABBITMQ_USERNAME=${RABBITMQ_USERNAME} \
-e PASSWORD=${RABBITMQ_PASSWORD} \
-e QUEUES_REGEXES=${QUEUES_REGEXES} \
-e EXCHANGES_REGEXES=${EXCHANGES_REGEXES} \
-e NR_LICENSE_KEY=${NR_LICENSE_KEY} \
newrelic-rabbitmq-agent:test

Origin Read me for base of the image:
